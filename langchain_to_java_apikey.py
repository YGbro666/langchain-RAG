...
BACKEND_URL = "http://0.0.0.0:8091"
LANGCHAIN_URL = "http://0.0.0.0:6006"
Ollama_URL = "http://localhost:11434"

# 添加新的常量定义
DASHSCOPE_API_KEY = "sk-99eb3947fed441208958823bfd8fe7b2"  # 需要替换为实际的API密钥
...
def agent(request: ChatRequest):
    try:
        ...
        # 使用 ConversationBufferMemory 替代 AgentTokenBufferMemory
...
请保持对话的连贯性，并参考历史对话记录来提供更好的回答。"""
        ...
        # 执行agent
        result = agent_executor({"input": request.prompt})

        # 提取最终输出
        final_response = result["output"] if isinstance(result, dict) else str(result)

        # 保存聊天历史
        chat_history_manager.add_chat(
            prompt=request.prompt, response=final_response, status="success"
        )
